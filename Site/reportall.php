...
$casenumber = array();
while ($row = pg_fetch_assoc($resultCases)) {
	$casenumber[] = $row['cid'];
}
...
for ($j = 0; $j < $casecount; $j++) {
	...
	$sqlCase = "SELECT * FROM cases NATURAL JOIN patient WHERE cid = '{$casenumber[$j]}'";
	//grab all diagnosis info
	$sqlDiag = "SELECT * FROM patient NATURAL JOIN cases INNER JOIN diagnosis ON diagnosis.did = cases.did WHERE cases.cid = '{$casenumber[$j]}'";
	$sqlNurse = "SELECT fname, lname, licno FROM employee NATURAL JOIN cases where cid = '{$casenumber[$j]}'";
	$sqlDoctor = "SELECT * FROM employee NATURAL JOIN diagnosis INNER JOIN cases ON diagnosis.did = cases.did WHERE cases.cid = '{$casenumber[$j]}'";
